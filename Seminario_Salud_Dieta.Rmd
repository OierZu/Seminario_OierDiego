...
```
```{r}
# Suponiendo que copia_nueva_tabla es tu tabla original
...
```{r}
# Convertir las columnas de la tabla a formato numérico
copia_nueva_tabla_num <- as.data.frame(lapply(copia_nueva_tabla, as.numeric))
...
```{r}
# Eliminar caracteres no numéricos o espacios en blanco en la columna "CT"
copia_nueva_tabla$CT <- gsub("[^0-9.]", "", copia_nueva_tabla$CT)

# Convertir a valores numéricos
copia_nueva_tabla$CT <- as.numeric(copia_nueva_tabla$CT)

# Generar un boxplot para la columna "CT" de la tabla 'copia_nueva_tabla'
boxplot(copia_nueva_tabla$CT, col = "green", main = "Boxplot de CT en copia_nueva_tabla")

# Obtener los datos de la columna CT de copia_nueva_tabla
datos <- copia_nueva_tabla$CT
...
x <- seq(min(datos), max(datos), length.out = 1000)  # Valores x para la curva de la campana de Gauss
densidad <- dnorm(x, mean = media, sd = desviacion)  # Calcula los valores de densidad en la distribución normal

# Graficar la campana de Gauss
plot(x, densidad, type = "l", col = "blue", lwd = 2,
     main = "Campana de Gauss (Distribución Normal)", 
...
```{r}
# Eliminar caracteres no numéricos o espacios en blanco
nueva_tabla$BTPAD <- gsub("[^0-9.]", "", nueva_tabla$BTPAD)

# Convertir a valores numéricos
nueva_tabla$BTPAD <- as.numeric(nueva_tabla$BTPAD)

# Genera un boxplot para la columna "BTPAD" de la tabla 'nueva_tabla'
boxplot(nueva_tabla$BTPAD, col = "green", main = "Boxplot de BTPAD en nueva_tabla")

# Obtener los datos de la columna BTPAD de nueva_tabla
datos <- nueva_tabla$BTPAD
...
# Calcular los percentiles 25, 50 y 75 en los datos
percentiles <- quantile(datos, c(0.25, 0.5, 0.75))

# Graficar líneas verticales en los percentiles
abline(v = percentiles, col = c("red", "green", "blue"), lty = 2)
...
# Ordenar datos por la columna V4 en orden ascendente
datos_filtrados <- datos_filtrados %>% 
  arrange(V4)

# Convertir V3 a factor para evitar errores si es numérico
datos_filtrados$V3 <- as.factor(datos_filtrados$V3)
